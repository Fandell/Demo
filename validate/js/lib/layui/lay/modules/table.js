...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports){
    ...
    if(options.url){ //Ajax请求
      ...
      $.ajax({
        ...
        ,success: function(res){
          ...
          //edit by lhr
          var countNum = 0;
          if(response.countName.indexOf(".") >= 0){
            //是result.count这样的格式的
            var objArr = response.countName.split(".");
            if(objArr.length == 2){
              countNum = res[objArr[0]][objArr[1]];
            }
          }else{
            //是count格式的
            countNum = response.countName;
          }
          that.renderData(res, curr, countNum), sort();
          options.time = (new Date().getTime() - that.startTime) + ' ms'; //耗时（接口请求+视图渲染）
          loadIndex && layer.close(loadIndex);
          typeof options.done === 'function' && options.done(res, curr, countNum);
        }
        ,error: function(e, m){
  ...
  Class.prototype.renderData = function(res, curr, count, sort){
    ...
    //edit by lhr
    //返回的列表，支持result.list和list这样的格式的 目前只支持单层和二层
    if(options.response.dataName.indexOf(".") >= 0){
      //是result.list这样的格式
      var arr = options.response.dataName.split(".");
      data = res[arr[0]][arr[1]] || [];
    }else{
      //是list这样的格式
    ...
    ,render = function(){ //后续性能提升的重点
      ...
      layui.each(data, function(i1, item1){
        ...
        if(item1 == null || item1.length === 0) return;
        if(!sort){
      ...
      if(Math.round(elemCell.prop('scrollWidth')) > Math.round(elemCell.outerWidth())){
        that.tipsIndex = layer.tips([
          '<div class="layui-table-tips-main" style="margin-top: -'+ (elemCell.height() + 16) +'px;'+ function(){
            ...
            ,elemCell.html()
          ,'</div>'
          ,'<i class="layui-icon layui-table-tips-c" style="top: -' + (elemCell.height() + 16) + 'px;">&#x1006;</i>'
        ].join(''), elemCell[0], {
          tips: [3, '']
          ,time: -1
          ,anim: -1
          ,maxWidth: (device.ios || device.android) ? 300 : elemCell.outerWidth()
          ,isOutAnim: false
